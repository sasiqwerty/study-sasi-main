...
The Fc region (the trunk of the Y shape) is composed of constant domains from the heavy chains. Its role is in modulating immune cell activity: it is where effector molecules bind to, triggering various effects after the antibody Fab region binds to an antigen. Effector cells (such as macrophages or natural killer cells) bind via their Fc receptors (FcR) to the Fc region of an antibody, while the complement system is activated by binding the C1q protein complex. IgG or IgM can bind to C1q, but IgA cannot, therefore IgA does not activate the classical complement pathway.
...
Optimer ligands are a novel class of antibody mimetics. These nucleic acid based affinity ligands are developed in vitro to generate specific and sensitive affinity ligands that are being applied across therapeutics, drug delivery, bioprocessing, diagnostics, and basic research.
